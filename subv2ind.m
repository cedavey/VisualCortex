...
if isscalar(subscript_vector)
   linear_index = subscript_vector;
   return;
end
if siz(end)==1 && length(subscript_vector)==(length(siz)-1)
   subscript_vector = [subscript_vector 1];
elseif siz(1)==1 && length(subscript_vector)==(length(siz)-1)
   subscript_vector = [1 subscript_vector];
end
if (size(subscript_vector, 2) ~= numel(siz))
    sizerror('subscripts are wrong length for stated array dimensions.');
end
