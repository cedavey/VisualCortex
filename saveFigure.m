...
%
% E.g. saveFigure( gcf, [], false, {'pdf','fig'} );
%      saveFigure( fh, figname, true, {'pdf','eps','fig'});
function saveFigure( figs, names, fillScreen, varargin )
   ...
   try
      ...
      str = sprintf( 'Failed testing figs for handle validity - might be doubles instead of handles? Continuing without test...' );
   ...
   for ff=1:length(figs)
      if isempty(names)
         ...
         % if fig name provided, and it has a file format on it, but
         % multiple file extensions have been provided, remove the extension
         figname = names{ff};
         if any(figname=='.') && find(figname=='.',1,'last') == numel(figname)-3
            if numel( varargin ) > 1
               names{ff} = figname( 1:find(figname=='.',1,'last')-1 );
            end
         end
      end
