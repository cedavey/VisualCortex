...
% Specify 'rad' or 'deg' by, for example, 
%     Zw = phasewrap(Z, 'deg')
% 
%  Zw = phasewrap(Z) 
%  Zw = phasewrap(Z,'deg')  
...
%% Input checking: 
   ...
   tmp = cellfun(@islogical, varargin);
   if any(tmp)
      iscolormap = varargin{tmp};
   end
   %% Perform mathematics: 
   ...
   Zw = mod(Zw + MaxAngle, 2*MaxAngle) - MaxAngle; 
   if iscolormap
      Zw = (Zw + pi)/(2*pi);
      % check if all rows in original were the same color, & don't change
      % them
      for zi=1:size(Z,2)
         z = Z(:,zi);
